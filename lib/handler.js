...
let RPC_URL = process.env.RPC_URL || 'https://ethereum-sepolia-rpc.publicnode.com';
if (!RELAYER_KEY) {
  try {
    const rt = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'build', 'runtime.json'), 'utf8'));
    RELAYER_KEY = rt.relayerKey;
    RPC_URL = rt.rpcUrl || RPC_URL;
  } catch {
...
async function joinCircle({ code, address, handle }) {
  ...
  const clean = String(handle || '').slice(0, 40).trim() || 'member';
  ...
  if ((await susu.balanceOf(address)) === 0n) {
    const top = await susu.credit(address, DEMO_TOPUP, 'DEMO-WELCOME');
    await top.wait();
  }
...
async function settleRound({ circleId }) {
  ...
  const missed = events.filter((e) => e.name === 'TurnMissed').map((e) => e.args[3]);
...
/// Stand-in for a confirmed mobile money deposit. See lib/momo.js for the real shape.
...
async function activity({ circleId }) {
  ...
  const [gaveIn, settled, missed, credited] = await Promise.all([
    ...
    susu.queryFilter(susu.filters.Credited(), from, 'latest'),
  ]);
  ...
  const shape = (e) => {
    ...
    if (e.fragment.name === 'TurnMissed') return { kind: 'missed', who: e.args[3], amount: 0, round: num(e.args[1]) };
    return { kind: 'topup', who: '', amount: num(e.args[1]), round: -1 };
  };
  ...
  return { rows: rows.slice(0, 40), now: Date.now(), credited: credited.length };
}
